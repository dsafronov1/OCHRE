...
def get_pcm_enthalpy(t_pcm, enthalpy_lut):
    ...
    if t_pcm < enthalpy_lut[0, 0] or t_pcm > enthalpy_lut[-1, 0]:
        raise ValueError(
            f"t_pcm {t_pcm} is outside the range of the LUT [{enthalpy_lut[0, 0]} to {enthalpy_lut[-1, 0]}]"
        )
...
class TankWithPCM(StratifiedWaterModel):
    ...
    def __init__(self, pcm_properties=DEFAULT_PCM_PROPERTIES, pcm_water_node=5, pcm_vol_fraction=0.5, **kwargs):
        ...
        if not (6.582730627258115e-08 <= self.pcm_vol_fraction <= 0.9999999999999725):
            raise ValueError(
                f"pcm_vol_fraction {pcm_vol_fraction} must be between (6.582730627258115e-08 and 0.9999999999999725) to ensure stability."
            )
        if self.pcm_vol_fraction < 0.01 or self.pcm_vol_fraction > 0.99:
            self.warn(
                f"pcm_vol_fraction {pcm_vol_fraction} is outside the recommended range (0.01 to 0.99). Results may be inaccurate."
            )
...
class TankWithMultiPCM(StratifiedWaterModel):
    ...
    def __init__(
        self,
        pcm_properties: dict,
        pcm_node_vol_fractions: dict[int, float] = {4: 0.5, 5: 0.5, 6: 0.5},
        **kwargs,
    ):
        # PCM node data
        ...
        self.pcm_mass_kg = None  # kg (total)
        ...
        print(
            f"PCM enthalpy LUT file: {pcm_file} for case ha:{self.ha:.2f} with sa_ratio:{self.pcm_properties['sa_ratio']:.2f}"
        )

        # pcm_cp cache between
        self._pcm_cp_cache: dict[tuple[float, ...], np.ndarray] = {}

        # will be populated in load_rc_data()
        self.pcm_mass_dict: dict[int, float] = {}  # node -> grams
        self.pcm_node_properties: dict[int, dict] = {}
        self._internal_nodes_order: list[str] | None = None
        ...
        # PCM results variables
        self.pcm_heat_to_water = None  # W

        # Initialize PCM enthalpy after PCM masses exist (created in load_rc_data())
        if self.pcm_mass_dict:
            masses_g = np.array([self.pcm_mass_dict[node] for node in self.pcm_water_nodes], dtype=float)
            self.enthalpy_pcm = np.interp(self.states[self.t_pcm_idx], self.key_temp, self.key_enthalpy) * masses_g  # J
        else:
            self.enthalpy_pcm = np.zeros(len(self.t_pcm_idx), dtype=float)

    def load_rc_data(self, **kwargs):
        # New format: rc_params is a tuple: (capacitances_dict, resistances_dict)
        cap, res = super().load_rc_data(**kwargs)
        self.rc_params = (cap, res)

        # Create a dictionary to store PCM mass, capacitance, and resistances for each node
        self.pcm_mass_dict = {}
        self.pcm_node_properties = {}

        total_pcm_volume = 0.0
        total_pcm_mass_g = 0.0
        original_volume = self.volume  # liters, for proper PCM volume computation

        # parameters to calculate pcm-pcm conductivity resistance
        self.tank_height = 4 * 0.3048  # m
        start_temp = kwargs.get("Setpoint Temperature (C)", 51.66666666666667)
        self.volume_m3 = self.volume / 1e3  # liters -> m^3
        self.tank_radius = math.sqrt(self.volume_m3 / (math.pi * self.tank_height))
        effective_area_ratio = 0.8
        length = self.tank_height / 12  # cell height
        water_heater_cross_area = math.pi * self.tank_radius**2
        effective_area = effective_area_ratio * water_heater_cross_area

        # Ensure WH node capacitances exist under canonical node names
        # Canonical naming assumed: "WH1".."WH12" in cap, and "AMB" external
        # PCM nodes will be added as "PCM{node}"
        for i, node in enumerate(self.pcm_water_nodes):
            idx = node - 1
            pcm_frac = self.pcm_node_vol_fractions[node]

            node_volume_L = original_volume * self.vol_fractions[idx]  # liters
            pcm_volume_L = node_volume_L * pcm_frac
            total_pcm_volume += pcm_volume_L

            # PCM mass in grams: density (g/cm^3) * liters * 1000 (cm^3/L)
            pcm_mass_g = self.pcm_properties["solid"]["pcm_density"] * pcm_volume_L * 1e3
            self.pcm_mass_dict[node] = pcm_mass_g
            total_pcm_mass_g += pcm_mass_g

            # Add PCM capacitance for this node (J/K): cp (J/g-K) * mass(g)
            pcm_cp = np.interp(start_temp, self.enthalpy_lut[:, 0], self.enthalpy_lut[:, 1])
            cap[f"PCM{node}"] = float(pcm_cp * pcm_mass_g)

            # Reduce the water node capacitance for this node by the PCM fraction
            wh_key = f"WH{node}"
            if wh_key not in cap:
                raise KeyError(f"Missing capacitance key '{wh_key}' in RC capacitances dict")
            cap[wh_key] *= 1.0 - pcm_frac

            # Update the water node’s volume fraction to remove the PCM volume fraction
            self.vol_fractions[idx] *= 1.0 - pcm_frac

            # Add the water-PCM thermal resistance for this node (K/W)
            ha = self.pcm_properties["h"] * pcm_volume_L * 1e-3 * self.pcm_properties["sa_ratio"]  # W/K
            res[(f"PCM{node}", f"WH{node}")] = float(1.0 / ha)

            # PCM-to-PCM thermal resistance for sequential PCM nodes (K/W)
            if i < len(self.pcm_water_nodes) - 1:
                next_pcm_node = self.pcm_water_nodes[i + 1]
                if next_pcm_node - node == 1:  # sequential
                    res[(f"PCM{node}", f"PCM{next_pcm_node}")] = float(
            ...
            self.pcm_node_properties[node] = {
                ...
                "ha": ha,
                "sa_ratio": self.pcm_properties["sa_ratio"],
                "h": self.pcm_properties["h"],
            }
            print(f"Node {node} pcm mass: {(pcm_mass_g) / 1000:.3e} kg")

        # Subtract the total PCM volume from the global water volume
        self.volume -= total_pcm_volume
        self.pcm_mass_kg = total_pcm_mass_g / 1000.0
        ...
        # Lock internal node ordering once, after PCM nodes have been added.
        # This must remain consistent across updates.
        self._internal_nodes_order = list(cap.keys())

        self.rc_params = (cap, res)
        return self.rc_params

    def update_rc_network(self, t_pcm, **kwargs):
        """Get the dynamic specific heat for each of the pcm nodes and update capacitances in rc network."""
        cap, res = self.rc_params
        ...
        for i, node in enumerate(self.pcm_water_nodes):
            pcm_node_mass_g = self.pcm_mass_dict[node]
            pcm_specific_heat = float(pcm_specific_heats[i])
            cap[f"PCM{node}"] = pcm_specific_heat * pcm_node_mass_g  # J/K

        self.rc_params = (cap, res)
        return self.rc_params

    def update_state_space_model(self, **kwargs):
        # Update only matrices/capacitances; do NOT rebuild names, nx/nu, or reorder nodes.
        if self._internal_nodes_order is None:
            raise RuntimeError("Internal node order not initialized. load_rc_data() must run before updates.")

        pcm_temps = self.next_states[self.t_pcm_idx]
        cap, res = self.update_rc_network(pcm_temps)

        internal_nodes = self._internal_nodes_order
        external_nodes = list(self.external_nodes)

        A_c, B_c = self.create_rc_matrices(cap, res, internal_nodes, external_nodes)

        # Capacitances aligned to internal node ordering
        self.capacitances = np.array([cap[n] for n in internal_nodes], dtype=float)

        # Update continuous and discrete matrices
        self.A_c, self.B_c, self.C, self.D = self.create_matrices((A_c, B_c))
        self.A, self.B = self.to_discrete()

        # Update output values (consistent with existing state/input vectors)
    ...
    def update_inputs(self, schedule_inputs=None):
        ...
        # get heat injections from PCM (W), positive means heat injected into water
        self.pcm_heat_to_water = self.get_pcm_heat_xfer()

        # Enforce conservation: PCM loses what water gains
        # Apply +Q to WH nodes, -Q to PCM nodes (no resizing of inputs_init)
        if self.h_pcm_idx:
            self.inputs_init[self.h_pcm_idx] += -self.pcm_heat_to_water  # PCM heat removal
        self.inputs_init[self.h_pcm_wh_idx] += self.pcm_heat_to_water  # Water heat addition
    ...
    def update_model(
        ...
        original_inputs_init=None,
    ):
        super().update_model(control_signal)

        masses_g = np.array([self.pcm_mass_dict[node] for node in self.pcm_water_nodes], dtype=float)
        enthalpy_state = np.interp(self.states[self.t_pcm_idx], self.key_temp, self.key_enthalpy) * masses_g
        enthalpy_next_state = np.interp(self.next_states[self.t_pcm_idx], self.key_temp, self.key_enthalpy) * masses_g
    ...
    def generate_results(self):
        ...
        if self.verbosity >= 3:
            results["Total PCM Enthalpy (J)"] = float(self.enthalpy_pcm.sum())
            results["Delta Total PCM Enthalpy (J)"] = float(self.delta_enthalpy_pcm.sum())
            results["Total PCM Heat Injected (W)"] = float(self.pcm_heat_to_water_rc_network.sum())

        if self.verbosity >= 6:
            for i, idx in enumerate(self.t_pcm_idx):
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} Temperature (C)"] = float(self.states[idx])
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} Water Temperature (C)"] = float(
                    self.states[self.t_pcm_wh_idx[i]]
                )
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} Enthalpy (J)"] = float(self.enthalpy_pcm[i])
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} Enthalpy Delta (J)"] = float(
                    self.delta_enthalpy_pcm[i]
                )
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} Heat Injected (W)"] = float(
                    self.pcm_heat_to_water_rc_network[i]
                )
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} Capacitance (J/K)"] = float(self.capacitances[idx])
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} h (W/m^2K)"] = float(
                    self.pcm_node_properties[self.t_pcm_wh_idx[i] + 1]["h"]
                )
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} volume (L)"] = float(
                    self.pcm_node_properties[self.t_pcm_wh_idx[i] + 1]["volume[L]"]
                )
                results[f"Water Tank PCM{self.t_pcm_wh_idx[i] + 1} sa_ratio"] = float(
                    self.pcm_node_properties[self.t_pcm_wh_idx[i] + 1]["sa_ratio"]
                )

            results["PCM Mass (kg)"] = float(self.pcm_mass_kg) if self.pcm_mass_kg is not None else 0.0
            results["Water Volume (L)"] = float(self.volume)
...
class TankWithMultiPCMExternal(StratifiedWaterModel):
    ...
    def __init__(
        ...
        *,
        ...
        pcm_segment_thickness_in: float = 0.005,  # in PCM thickness
        ...
        **kwargs,
        ...
        self.water_side_film_h = self.pcm_properties.get("film_h", water_side_film_h)
        ...
        self.insulation_thickness_m = float(insulation_thickness_in * IN_TO_M - self.pcm_thickness_m)
        ...
        self.enamel_u_value_W_per_m2_K = self.enamel_k_value / (
        ...
        self.steel_u_value_W_per_m2_K = self.steel_k_value / (
        ...
        self.insulation_u_value_W_per_m2_K = self.insulation_k_value / (
    ...
    def load_rc_data(self, **kwargs):
        ...
        if self.pcm_thickness_m / IN_TO_M < pcm_in_min:
            print(
                f"Warning: PCM thickness {self.pcm_thickness_m / IN_TO_M} inches is less than {pcm_in_min} inches. Clamping to {pcm_in_min} inches."
            )
            pcm_thickness_inches = pcm_in_min
        elif self.pcm_thickness_m / IN_TO_M > pcm_in_max:
            print(
                f"Warning: PCM thickness {self.pcm_thickness_m / IN_TO_M} is greater than {pcm_in_max} inches. Clamping to {pcm_in_max} inches."
            )
        ...
        def compute_ua_bypass_shell(res: dict) -> dict:
            """
            Compute UA contributions using only resistors connected to ambient.
            - Bypass UA: WH{i}<->AMB links
            - Shell UA : all other *<->AMB links
            ...
            for key, value in res.items():
                if not (isinstance(key, tuple) and len(key) == 2 and "AMB" in key):
                    continue
                ...
                other = key[0] if key[1] == "AMB" else key[1]
                if other.startswith("WH") and other[2:].isdigit():
                    ua_bypass += G
                else:
                    ua_shell += G
        ...
        if total_pcm_segments > max_recommended_segments:
            print(
                f"Warning: Total PCM segments ({total_pcm_segments}) exceeds recommended limit ({max_recommended_segments})"
            )
        ...
        print(
            f"  - Circumferential segments: {n_pcm_circumferential} (arc: {actual_circumferential_size * 1000:.2f} mm)"
        )
        ...
        cp_pcm = np.interp(start_T, self.enthalpy_lut[:, 0], self.enthalpy_lut[:, 1])
        ...
        for i in range(1, n_nodes + 1):
            ...
            res[(f"WH{i}", f"ENM{i}")] = 1.0 / (self.water_side_film_h * A_layer)
            # enamel
            cap[f"ENM{i}"] = C_en
            res[(f"ENM{i}", f"STL{i}")] = R_cond(k_en, r_w, r_en)
            # steel
            cap[f"STL{i}"] = C_st
            ...
            for r in range(1, n_pcm_radial + 1):
                ...
                for c in range(1, n_pcm_circumferential + 1):
                    for a in range(1, n_pcm_axial + 1):
                        ...
                        # Capacitance
                        cap[seg_id] = mass_segment * cp_pcm * 1e3  # cp_pcm in in J/g-C
                        ...
                        if r == 1:
                            # Connect to steel
                            res[(f"STL{i}", seg_id)] = R_radial / 2  # Half resistance to segment center
                        else:
                            # Connect to inner radial neighbor
                            inner_seg_id = f"PCM{i}-{r - 1}-{c}-{a}"
                            res[(inner_seg_id, seg_id)] = R_radial

                        # Connect to outer radial neighbor or insulation
                        if r == n_pcm_radial:
                            # Connect to insulation
                            res[(seg_id, f"INS{i}")] = R_radial / 2  # Half resistance to segment center
                        else:
                        ...
                        if c < c_next or (c == n_pcm_circumferential and c_next == 1 and seg_id != next_seg_id):
                            res[(seg_id, next_seg_id)] = R_circumferential
                        ...
                        if a < n_pcm_axial:
                            ...
                            res[(seg_id, next_axial_seg_id)] = R_axial_segment
                        ...
                        if include_axial and i < n_nodes:
                            ...
                            res[(seg_id, next_layer_seg_id)] = R_layer_axial

            # insulation
            cap[f"INS{i}"] = C_ins
            R_ins_r = R_cond(k_ins, r_pcm, r_ins)
            A_out = 2 * math.pi * r_ins * L_layer + 2 * math.pi * r_ins**2 / n_nodes
            res[(f"INS{i}", "AMB")] = R_ins_r + 1.0 / (h_ext * A_out)

            # axial conduction for non-PCM layers
            if include_axial and i < n_nodes:
                ni = i + 1
                res[(f"ENM{i}", f"ENM{ni}")] = L_layer / (k_en * A_vert_en)
                res[(f"STL{i}", f"STL{ni}")] = L_layer / (k_st * A_vert_st)
                res[(f"INS{i}", f"INS{ni}")] = L_layer / (k_ins * A_vert_ins)

        # remove original water->amb resistances
        for i in range(1, n_nodes + 1):
            res.pop((f"WH{i}", "AMB"), None)
            res.pop(("AMB", f"WH{i}"), None)
        ...
        tank_ua_values = compute_ua_bypass_shell(res)
    ...
    def update_rc_network(self, t_pcm, states, current_schedule, **kwargs):
        ...
        for i in range(1, self.n_nodes + 1):
            key = (f"WH{i}", f"ENM{i}")
            rev_key = (f"ENM{i}", f"WH{i}")
            if key in res:
                res[key] = float(film_conv_resistance)
            elif rev_key in res:
                res[rev_key] = float(film_conv_resistance)
        ...
        for i, node in enumerate(self.pcm_mass_dict.items()):
            ...
            cap[index] = pcm_specific_heat * pcm_node_mass * 1e3
        self.rc_params = (cap, res)
        return self.rc_params
    ...
    def update_state_space_model(self, states, current_schedule, **kwargs):
        ...
        pcm_temps = self.next_states[self.t_pcm_idx]
        all_cap, all_res = self.update_rc_network(pcm_temps, states, current_schedule)
        internal_nodes = self._internal_nodes_order if self._internal_nodes_order is not None else list(all_cap.keys())
        external_nodes = list(self.external_nodes)

        # Recompute the state-space matrices
        A_c, B_c = self.create_rc_matrices(all_cap, all_res, internal_nodes, external_nodes)

        # Update capacitances aligned to internal node ordering.
        self.capacitances = np.array([all_cap[n] for n in internal_nodes], dtype=float)
    ...
    def calculate_film_convective_heat_transfer_coefficient(self):
        """
        ...
        T_wall_C = (
            ...
            if getattr(self, "t_en_idx", None) is not None
    ...
    def generate_results(self):
        ...
        if self.verbosity >= 6:
            # water nodes
            if type(self.water_side_film_h) is np.ndarray:
                for i, idx in enumerate(self.t_wh_idx):
                    results[f"Film Tank {self.state_names[idx]} Heat Transfer Coefficient (W/m^2-K)"] = (
                        self.water_side_film_h[i]
                    )
            else:
                for i, idx in enumerate(self.t_wh_idx):
                    results[f"Film Tank {self.state_names[idx]} Heat Transfer Coefficient (W/m^2-K)"] = (
                        self.water_side_film_h
                    )
            # pcm nodes
            for i, idx in enumerate(self.t_pcm_idx):
